              ...
              withCredentials([string(credentialsId: "${OCP_CREDENTIAL}", variable: 'OCP_SERVICE_TOKEN')]) {
                ...
                if (isDCExists == 1) {
                  sh"""
                    ...
                    oc set probe dc/${PROJECT_NAME} --readiness --get-url=http://:8080/q/health/ready --initial-delay-seconds=30 --token=${OCP_SERVICE_TOKEN} $target_cluster_flags
                    oc set probe dc/${PROJECT_NAME} --liveness --get-url=http://:8080/q/health/live --initial-delay-seconds=30  --token=${OCP_SERVICE_TOKEN} $target_cluster_flags
